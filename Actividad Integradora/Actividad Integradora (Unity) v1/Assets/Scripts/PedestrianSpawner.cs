...
public class PedestrianSpawner : MonoBehaviour
{
    ...
    public string dataUrl = "http://127.0.0.1:5000/pedestrians";
    ...
    IEnumerator FetchCarData()
    {
        using (UnityWebRequest webRequest = UnityWebRequest.Get(dataUrl))
        {
            ...
            else
            {
                ...
                string wrappedJson = "{\"pedestrians\":" + jsonResponse + "}";
                TaxiList carList = JsonUtility.FromJson<TaxiList>(wrappedJson);

                Debug.Log($"Pedestrians Count: {carList.pedestrians.Count}");
    ...
    {
        foreach (var car in carList.pedestrians)
        {

            // Instantiate the selected car model
            GameObject instantiatedPassenger = Instantiate(pedestrianPrefab, Vector3.zero, Quaternion.identity);
